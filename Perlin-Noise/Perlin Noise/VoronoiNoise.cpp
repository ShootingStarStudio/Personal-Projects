...
VoronoiNoise::VoronoiNoise(unsigned int randseed) {
	seed = randseed;
	//seeding the random engine
	std::default_random_engine engine(seed);
	//make vector size of 256
	p.resize(256);									//assume it creates a 256 distribution
...
double VoronoiNoise::fade(double t) {		//ease curve
	return t * t * t * (t * (t * 6 - 15) + 10);
	//quintic interpolation instead of smoothstep, reduces artefacts
	//old fade curve: 6t^5-15t^4-10t^3
}
double VoronoiNoise::lerp(double t, double a, double b) {
	return a * (1 - t) + b * t;
}
double VoronoiNoise::grad(int hash, double x, double y, double z) {
	int h = hash & 15;
